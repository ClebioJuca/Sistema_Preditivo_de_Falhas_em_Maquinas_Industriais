...
inline const uint8_t* ssd1306_get_font(uint8_t character)
{
    if (character >= 0x20 && character <= 0x7F) {
        return font8x8_basic_tr[character];
    }

    // Latin-1 estendido
    if (character >= 0xA0 && character <= 0xFF) {
        return font8x8_ext_latin_tr[character - 0xA0];
    }

    // fallback: espaço
    return font8x8_basic_tr[0x20];
}
...
void ssd1306_draw_char(uint8_t *ssd, int16_t x, int16_t y, uint8_t character) {
    ...
    const uint8_t* bitmap = ssd1306_get_font(character);

    for (int i = 0; i < 8; i++) {
        ssd[(y * ssd1306_width) + x + i] = bitmap[i];
    }
}

static inline uint8_t utf8_to_latin1(char **s)
{
    uint8_t c = (uint8_t)**s;
    ...
    // UTF-8 Latin-1 (0xC2 / 0xC3)
    if (c == 0xC2 || c == 0xC3) {
        uint8_t next = (uint8_t)(*s)[1];
        (*s) += 2;

        if (c == 0xC2) return next;
        if (c == 0xC3) return next + 0x40;
    }

    // Não suportado → espaço
    (*s)++;
    return 0x20;
}
...
void ssd1306_draw_string(uint8_t *ssd, int16_t x, int16_t y, char *string) {
    ...
    while (*string) {
        uint8_t ch = utf8_to_latin1(&string);
        ssd1306_draw_char(ssd, x, y, ch);
        x += 8;
    }
}
